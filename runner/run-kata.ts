...
async function main() {
  ...
  const preamble = stateless
    ? ""
    : loadAdlerianPreamble(DOJO_ROOT, "dev", kata, ring, { topN: 5, topNSeeds: 4, kataType: kataYaml.type === "fullstack" ? "fullstack" : "backend", searchQuery: description + " " + architecture });
  if (preamble) {
